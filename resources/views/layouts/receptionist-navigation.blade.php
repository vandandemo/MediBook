...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 shadow-sm">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex items-center">
                    ...
                    <x-nav-link :href="route('receptionist.patients.index')" :active="request()->routeIs('receptionist.patients.*')">
                        {{ __('Patients') }}
                    </x-nav-link>
                    <x-nav-link :href="route('receptionist.appointments.index')" :active="request()->routeIs('receptionist.appointments.*')">
                        {{ __('Appointments') }}
                    </x-nav-link>
                    <x-nav-link :href="route('receptionist.billing.index')" :active="request()->routeIs('receptionist.billing.*')">
                        {{ __('Billing') }}
                    </x-nav-link>
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                <div class="font-medium text-base text-gray-800">{{ $receptionist->name }}</div>
                <div class="font-medium text-sm text-gray-500">{{ $receptionist->email }}</div>
            </div>

            <div class="mt-3 space-y-1">
                <x-responsive-nav-link :href="route('receptionist.profile.edit')">
                    {{ __('Profile') }}
                </x-responsive-nav-link>
